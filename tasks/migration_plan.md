...
| `steps/parser_step.py` (FB2/EPUB chapter extraction) | new `lib/service/pipeline/chapter_parser_service.dart` (drives foliate-js via the existing `EpubPlayer` JS bridge) | Don't port the Python parser, and don't extend Anx's metadata-only importer. Instead, run a one-shot pass on the **first open** of a book: walk the foliate-js TOC, fetch plain text per `href`, write one row per chapter into `tb_source_chapters`. Idempotent — skips when `countByBookId(bookId) > 0`. |
...
1. **Iteration 1 — discovery + GlossaryTerm slice.** DAO + model + Riverpod provider for glossary terms only. Verify the existing Anx schema accepts our new tables. No UI. *(landed)*
2. **Iteration 2 — chapter tables + status enum.** Add `tb_source_chapters`, `tb_target_chapters`, `ChapterStatus` enum. DAO + provider. No UI. *(landed)*
3. **Iteration 3 — first-open chapter parser.** When the user opens a book for the first time, walk the foliate-js TOC and populate `tb_source_chapters` (one row per chapter, status `parsed`). Background-driven, ненавязчивый прогресс-индикатор в drawer'е. EPUB / FB2 only. No translation yet. Source: `D:\Projects\NovelTranslator\steps\parser_step.py`, but reusing foliate-js for actual content extraction. *(landed)*
4. **Iteration 4 — translation prompts + single-chapter manual translate.** Port `COMPACT_TRANSLATOR_SYSTEM_PROMPT` from `D:\Projects\NovelTranslator\modules\services\prompts\translation.py` into `lib/service/ai/prompt_generate.dart`; build `lib/service/pipeline/translator_service.dart` for one chapter (single LLM call, JSON output, parse `title` + `translation`). Bind to a developer-only debug button in `lib/page/settings_page/developer/developer_options_page.dart`. Operates on rows in `tb_source_chapters` populated by Iteration 3. Glossary RAG-lite, genre presets, audit passes still deferred.
